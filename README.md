...
**⚠️ 重要注意事項：**
- **純前端技術限制**：加密的 PDF 解鎖後將轉為高品質影像（300 DPI PNG 無損），文字將無法選取或搜尋
- 這是目前純前端技術的限制，如需保留文字層，請參考 [後端實現指南](BACKEND_GUIDE.md)
- 處理時間取決於頁數和複雜度
...
5. **PDF 解鎖限制**：純前端無法保留加密 PDF 的文字層，如需此功能請參考 [後端實現指南](BACKEND_GUIDE.md)
...
├── BACKEND_GUIDE.md                 # 後端實現指南
└── THIRD_PARTY_LICENSES.md          # 第三方授權說明
```
...
如果您需要解鎖加密 PDF 並**保留文字層**，請參考：

📖 **[後端實現指南](BACKEND_GUIDE.md)**

內容包含：
- 完整的 Python FastAPI + pikepdf 實現範例
- 免費部署方案（Render.com / Railway.app）
- 安全性最佳實踐
- Docker 部署設定
- 前後端整合範例
